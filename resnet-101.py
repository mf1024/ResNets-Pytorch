...
model_resnet101 = ResNet(class_num = NUM_CLASSES, is_bottleneck_resnet = True).to(DEVICE)
optimizer = torch.optim.Adam(params = model_resnet101.parameters(), lr = LEARNING_RATE)
...
layers_debug(model_resnet101)

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

print(f"number of resnet101 params {count_parameters(model_resnet101)}")
...
for epoch in range(NUM_EPOCHS):
    ...
    model_resnet101 = model_resnet101.train()
    ...
    for batch_idx, batch in enumerate(data_loader_train):
        ...
        y_prim = model_resnet101.forward(x)
    ...
    with torch.no_grad():
        ...
        model_resnet101 = model_resnet101.eval()
        ...
        torch.save(model_resnet101, last_model_path)
        if epoch_test_accuracy > best_test_accuracy:
            best_test_accuracy = epoch_test_accuracy
            torch.save(model_resnet101, best_model_path)
